...
def output_to_target(output, width, height):
    ...
    for i, o in enumerate(output):
        if o is not None:
            o = o.cpu().numpy()  # Ensure the tensor is on CPU
            for pred in o:
                box = pred[:4]
                w = (box[2] - box[0]) / width
...
def plot_images(images, targets, paths=None, fname='images.jpg', names= ['tank'], max_size=640, max_subplots=16):
        ...
        if len(targets) > 0:
            ...
            for j, box in enumerate(boxes.T):
                ...
                if cls >= len(names) or cls < 0:
                    print(f"Warning: Found class {cls}, but it's out of the names list range. Skipping this box.")
                    continue
